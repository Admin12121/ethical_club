...
const DitheredLogo = ({
  ...
  const startLoop = useCallback(() => {
    ...
    const tick = () => {
      ...
      drawParticles(ctx, sys, invert, rect.width, rect.height, dpr);
      if (needsMore) {
  ...
  const rebuild = useCallback(
    async (src: string) => {
      ...
      try {
        ...
        systemRef.current = initParticles(positions, s, ds, ox, oy);
        startLoop();
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      const sys = systemRef.current;
      if (sys) drawParticles(ctx, sys, invert, rect.width, rect.height, dpr);
    ...
    const onPointerUp = (e: PointerEvent) => {
      ...
      if (e.pointerType !== 'mouse') cursorRef.current.active = false;
      startLoop();
    };
  ...
  }, [invert, startLoop, rebuild, imageSrc]);
